...
def collect_files(data_path,section):
  ...
  if ('NS' == section):
    for f in allfiles:
      if re.search(r'grid\d+_NS_(up|down|left|right|back|front)_\w*\.silo$',f) or \
         re.search(r'grid\d+_centeral_box_\w*\.silo$',f):
        files_name.append(f)
  
  elif ('NS_surrounding' == section):
    for f in allfiles:
      if re.search(r'grid\d+_NS_surrounding_(up|down|left|right|back|front)_\w*\.silo$',f):
        files_name.append(f)
