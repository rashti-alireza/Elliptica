...
def collect_files(data_path,section):
  ...
  if ('BH' == section):
    for f in allfiles:
      if re.search(r'grid\d+_BH_surrounding_(up|down|left|right|back|front)_\w*\.silo$',f):
        files_name.append(f)
...
def pars_arguments():
  ...
  if re.search(r'^(?i)s(calar)?',str(args.object_type)):
    object_type = 'Pseudocolor'
    
  elif re.search(r'^(?i)v(ector)?',str(args.object_type)):
    object_type = 'Vector'
