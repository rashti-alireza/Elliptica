...
void Lorentz_boost(Transformation_T *const t,const double *const in,double *const out)
{
  ...
  const double G_D1D1 = 1+(gamma-1)*SQR(Bx)/B2;
  const double G_D1D2 = (gamma-1)*Bx*By/B2;
  const double G_D1D3 = (gamma-1)*Bx*Bz/B2;
  const double G_D2D2 = 1+(gamma-1)*SQR(By)/B2;
  const double G_D2D3 = (gamma-1)*By*Bz/B2;
  const double G_D3D3 = 1+(gamma-1)*SQR(Bz)/B2;
