...
void sns_partial_derivatives_fields(Grid_T *const grid)
{
  ...
  for (p = 0; p < np; ++p)
  {
    ...
    /* u0 derivatives */
    sns_update_derivative_u0(patch);
    
    /* K derivatives */
    sns_update_derivative_K(patch);
    ...
    /* B0^i*/
    sns_update_derivative_B0_U0(patch);
    sns_update_derivative_B0_U1(patch);
    sns_update_derivative_B0_U2(patch);
