...
static Grid_T *make_next_grid_using_previous_grid(Grid_T *const grid_prev)
{
  ...
  */
  ...
  /* updating enthalpy */
  sbh_update_enthalpy_and_denthalpy(grid_prev);
  ...
  sbh_update_stress_energy_tensor(grid_next);
...
static void find_BH_NS_Omega_force_balance_eq(Grid_T *const grid)
{
  ...
  //sbh_update_Aij(patch); Do we need this??
}
...
static double Euler_eq_const_rootfinder_eq(void *params,const double *const x)
{
  ...
  return sbh_NS_baryonic_mass(par->grid,x[0]) - par->NS_baryonic_mass;
}
...
static void find_NS_surface_Ylm_method_CS(Grid_T *const grid,struct Grid_Params_S *const GridParams)
{
  ...
  root->x_gss     = &guess;
  root->params    = par;
  root->f[0]      = sbh_NS_surface_enthalpy_eq;
  root->df_dx[0]  = sbh_NS_surface_denthalpy_dr;
  //root->verbose   = 1;
  //root->FD_Right  = 1;
  plan_root_finder(root);
...
static double sbh_NS_surface_enthalpy_eq(void *params,const double *const x)
{
...
static double sbh_NS_surface_denthalpy_dr(void *params,const double *const x,const unsigned dir)
{
  assert(dir == 0);
  const struct NS_surface_RootFinder_S *const pars = params;
  Patch_T *const patch0  = pars->patch;/* supposed to find in this patch */
